 ...
 */
 ...
 contract uniLock  {
    ...
    function getGivenAmount(address _address) public view returns (uint){
     ...
     function recover(bytes32 hash, bytes memory signature,address _address, uint _amount)
    public
    pure
    returns (address)
  {
    require(hash == getMessageHash(_address,_amount),'Invalid Message hash');
  ...
  function getMessageHash(address _address, uint _amount)
